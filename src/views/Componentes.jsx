...
export const Cabecera = ({ ir, cantidad = 0, cerrarSesion }) => (
  <header className="cabecera">
    <Logo claro />
    <nav>
      <button type="button" onClick={() => ir('menu')}>Menú</button>
      <button type="button" onClick={() => ir('seguimiento')}>Seguimiento</button>
      <button type="button" onClick={() => ir('historial')}>Historial</button>
      <button type="button" className="boton-carrito" onClick={() => ir('carrito')}>
        🛒 Carrito <b>{cantidad}</b>
      </button>
      <button type="button" className="salir" onClick={cerrarSesion}>Salir</button>
    </nav>
  </header>
);
